...
def trouver_coordonnees_centroide(liste_coordonne):
    ...
    nb_dimension = liste_coordonne.ndim
...
def trouver_point_centre(liste):
    ...
    for i in range(len(liste)):
        ...
        for k in range(len(liste)):
            point_1=liste[i]
            point_2=liste[k]
            s=s+distance_euclidienne(point_1, point_2)
...
def trouver_point_plus_loin(liste):
#la fonction va determiner le point le plus eloigné du centroid de la liste
    centroid=trouver_coordonnees_centroide(liste)
    k=0 # le rang du point le plus loin
    pointplusloin=liste[k]
    for i in range(len(liste)): # si la distance entre le point de rang k et le centroid est inferieur à la distance entre le point de rang i et le centroid, alors k revient i
        point_1=liste[k]
        point_2=centroid
        point_3=liste[i]
...
def creer_nouvelleliste(liste,pointplusloin):
    ...
    centroid=trouver_coordonnees_centroide(liste)
    for point in liste:
        if distance_euclidienne(point,pointplusloin) <= distance_euclidienne(centroid,pointplusloin):
...
def balltree(precision,liste):
    ...
    for tour in range(precision): # on divise les listes autant de fois quon veut etre précis
        pointplusloin=trouver_point_plus_loin(liste_a_diviser)
        centroid=trouver_coordonnees_centroide(liste_a_diviser)
        nouvelleliste=creer_nouvelleliste(liste_a_diviser,pointplusloin)
        liste_complementaire=creer_liste_complementaire(liste_a_diviser,nouvelleliste) # on a fractionné la liste initiale en 2 listes qui regroupent ensemble tout les points
    ...
    return liste_crees # on renvoie une liste contenant toute les sous listes de la liste initiale
...
def prediction(liste_centroid,nouveau_point): # on cherche le centroid le plus proche du nouveau point
    k=0
    centroid_proche=liste_centroid[k]
    for i in range(len(liste_centroid)):
        point_1=liste_centroid[k]
        point_2=liste_centroid[i]
        if distance_euclidienne(point_1,nouveau_point) > distance_euclidienne(point_2,nouveau_point):
            k=i
