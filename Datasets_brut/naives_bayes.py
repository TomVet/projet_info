...
def calcul_esp_categorie_fonction_classe(points): # renvoie lesperance de chaque catégorie en fonction de sa classe
...
def calcul_var_categorie_fonction_classe(points):
    dim_point = len(points[0]) - 1
    variances = []
    variance_par_classe=[]
    classes=set(point[-1] for point in points)
    ...
    for classe in classes:
        for rang in range(dim_point):
            for point in points:
                if point[-1]==classe:
                    compteur+=1
                    somme = somme + (point[rang]-calcul_esp_categorie_fonction_classe(points)[0][1])**2
...
def calcul_proba_categorie_sachant_classe(point,points,categorie,classe):
    esp=calcul_esp_categorie_fonction_classe(points)[classe][0][categorie]
    var=calcul_var_categorie_fonction_classe(points)[classe][0][categorie]
    proba=exp((-((point[categorie]-esp)**2)/(2*var**2)))/sqrt(2*float(pi)*var**2)
    return proba

def creer_intervalle(points,categorie):#categorie est un chiffre (la xème catégorie etudiée)
    points_categorie=[]
    for point in points:
